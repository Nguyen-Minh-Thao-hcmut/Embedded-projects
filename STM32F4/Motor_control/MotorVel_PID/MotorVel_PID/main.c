...
int main()
{
		 ...
		 if (bDataAvailable == 1)
    {
      ...
      {
        tProcess = (tProcess == CRUN)? STOP : tProcess;
      }
      else
      {
        tProcess = NONE;
    ...
    case SPID:
      ...
      tPIDControl.dKd = (float)g_strData[4] + (float)g_strData[5] / (pow((float)10, (float)g_strData[6])); // Kd = 30.0005 thi g_strData[4] = 30, g_strData[5] = 5, g_strData[5] = 4
			tPIDControl.dFrequency = (uint16_t) (g_strData[7]*10000 + g_strData[8]*100 + g_strData[9]); // fPwm = 21325 Hz thi g_strData[7] = 2, g_strData[8] = 13, g_strData[9] = 25
    ...
    case CRUN:
      bDataAvailable = 0;
			...
			{
				int index;
				for (index = 0; index < (g_nIndex - 1); index++)
				{
				memcpy(g_strTxCommand, g_strCommand, 4);
				memset(g_nTxOption, '\0', 3);
				g_nTxData[2*index] = (tPIDControl.nActVelSample[index]&0xFF00) >> 8;
				g_nTxData[2*index + 1] = (tPIDControl.nActVelSample[index]&0xFF);
				bDataAvailable = 1;
