...
void MotorInit(uint16_t PWM_Freq)
{
	...
	uint32_t ARR_Value;
	ARR_Value = (uint32_t)((_TIM3_CLK_/PWM_Freq) - 1);
	TIM3 -> ARR = ARR_Value;
	TIM3 -> CCR1 = 0;
...
{
	uint32_t ARR_Value1;
	ARR_Value1 = (uint32_t)((_TIM3_CLK_/dFrequency) - 1);
	TIM3 -> ARR = ARR_Value1;
	TIM3 -> CCR1 = 0;
}
void MotorReset(uint16_t dFrequency)
{
	...
	TIM3 -> ARR = (uint32_t)((_TIM3_CLK_/3000)-1);
	TIM3 -> CCR1 = 0;
	TIM_SetCounter(TIM3, 0);
	TIM_SetCounter(TIM4, _TIM4_CNT_VAL_INIT_);
	TIM_SetCounter(TIM2, 0);
...
void MotorSetDuty(uint32_t nDuty)
{
	TIM3->CCR1 = nDuty ;
}

float ConvertPulseToVel(uint32_t nPulse)
{
	float nVel = ((float)nPulse * 60 * (1000/ _SAMPLE_TIME_ ) )/(_PULSE_MODE_ * _CHANNEL_NUM_ * _PULSE_PER_CHANNEL_ * _RATIO_ ); //RPM VD 10ms -> 10 xung => 10  ms = 0.05 vòng => 1 phút = 0.05 * (60 000/10)
	return nVel;
}
...
uint32_t ConvertVelToPulse(float nVel, int* nDir)
{
	...
	nPulse = ((abs(nVel) * 10 )*( 1 * 2 * 3* 45)/ 60000);
...
void MotorMoveVel(float nVel)
{
	...
	g_nActPulse = nPulse - g_nPrePulse;
	g_nPrePulse = nPulse;
	g_nCmdPulse = ConvertVelToPulse(nVel,&nDir);
	
	g_nDutyCycle = (int32_t)PIDCompute(&tPIDControl, g_nCmdPulse, g_nActPulse, 0.01 );
	if (g_nDutyCycle >= 0)
	{
		MotorSetDir(1);
	...
	tPIDControl.nActVelSample[g_nIndex] = (int16_t)ConvertPulseToVel(g_nActPulse);
	g_nIndex++;
